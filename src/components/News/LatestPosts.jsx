...
const LatestPosts = () => {
  ...
  return (
    <div className="container mx-auto p-2">
        <h1 className='md:py-4 py-2 md:text-3xl text-4xl text-gray-800 font-bold md:text-center text-start'>Latest News</h1>
      {posts.length === 0 ? (
        <p className="text-center text-gray-600">No posts available.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:px-24">
          {posts.map((post) => (
            <div key={post.postId} className="bg-white rounded-lg shadow-lg hover:shadow-2xl transition-shadow hover:-translate-y-1 hover:scale-105 duration-1500">
              <img 
